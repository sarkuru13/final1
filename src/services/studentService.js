...
/**
 * Fetches a single student document based on their Appwrite Auth User ID.
 * @param {string} userId The Appwrite Auth user ID.
 * @returns {Promise<Object|null>} A promise that resolves to the student document or null.
 */
export async function getStudentByUserId(userId) {
  try {
    const response = await databases.listDocuments(
      DATABASE_ID,
      STUDENT_COLLECTION_ID,
      [Query.equal('userId', userId), Query.limit(1)] // Find the document where 'userId' matches
    );
    if (response.documents.length > 0) {
      return response.documents[0]; // Return the first and only matching document
    }
    return null; // Return null if no student is found
  } catch (error) {
    console.error('Failed to fetch student by user ID:', error.message);
    throw new Error('Failed to fetch student by user ID: ' + error.message);
  }
}
...
export async function createStudent(studentData) {
  try {
    ...
    // Attempt to create user in Appwrite Users service and link it
    try {
      ...
      // Update the student document with the userId, without changing permissions.
      await databases.updateDocument(
        DATABASE_ID,
        STUDENT_COLLECTION_ID,
        response.$id,
        {
          userId: userId, // Store the userId in the student document
        }
        // NOTE: By omitting the permissions parameter here, we ensure they are not changed from the initial admin-only set.
      );
      console.log('✅ User created and linked to student:', userId);
    } catch (userErr) {
      console.warn('⚠️ User creation/linking failed, but student data is stored:', userErr);
    }
    return response;
  } catch (error) {
...
export async function updateStudent(studentId, studentData) {
  try {
    ...
    const response = await databases.updateDocument(
      ...
      // NOTE: Permissions are intentionally omitted here to avoid overwriting them on update.
    );
    return response;
  } catch (error) {
    throw new Error(error.message);
